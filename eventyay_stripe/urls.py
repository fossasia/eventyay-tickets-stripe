    ...
    re_path(
        ...
        include(
            [
                ...
                re_path(
                    r"^return/(?P<order>[^/]+)/(?P<hash>[^/]+)/(?P<payment>[0-9]+)/$",
                    ReturnView.as_view(),
                    name="return",
                ),
                re_path(r"^sca/(?P<order>[^/]+)/(?P<hash>[^/]+)/(?P<payment>[0-9]+)/$", ScaView.as_view(), name="sca"),
                re_path(
                    r"^sca/(?P<order>[^/]+)/(?P<hash>[^/]+)/(?P<payment>[0-9]+)/return/$",
                    ScaReturnView.as_view(),
                    name="sca.return",
                ),
            ]
        ),
...
urlpatterns = [
    re_path(
        r"^control/event/(?P<organizer>[^/]+)/(?P<event>[^/]+)/stripe/disconnect/$",
        oauth_disconnect,
        name="oauth.disconnect",
    ),
    re_path(
        r"^control/organizer/(?P<organizer>[^/]+)/stripeconnect/$",
